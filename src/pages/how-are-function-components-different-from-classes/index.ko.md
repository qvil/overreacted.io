---
title: 함수형 컴포넌트는 클래스와 어떻게 다른가요?
date: '2019-03-03'
spoiler: 그들은 완전히 다른 포켓몬입니다. 
---

리액트 함수형 컴포넌트는 리액트 클래스와 어떻게 다릅니까?

그동안, 표준적인 대답은 클래스가 더 많은 기능(예: state)에 대한 접근을 제공한다는 것입니다. [Hooks](https://reactjs.org/docs/hooks-intro.html)과 함께라면, 그건 더이상 맞지 않습니다.

어쩌면 그들 중 하나가 성능이 더 좋다고 들었을 것입니다. 어떤 것인가요? 벤치마크의 많은 부분은 [결함](https://medium.com/@dan_abramov/this-benchmark-is-indeed-flawed-c3d6b5b6f97f?source=your_stories_page---------------------------)이 있습니다. 그래서 저는 그것들([drawing conclusions](https://github.com/ryardley/hooks-perf-issues/pull/2))로부터 결론을 내릴 때 조심할 것입니다. 성능은 주로 당신이 함수나 클래스를 선택는 것이 아니라 코드가 수행하는 작업에 기반합니다. 저희의 관점에서 성능 차이는 무시할만 하지만 최적화 전략은 약간 [다릅니다](https://reactjs.org/docs/hooks-faq.html#are-hooks-slow-because-of-creating-functions-in-render).

두 경우 모두 [권장하지 않습니다.](https://reactjs.org/docs/hooks-faq.html#should-i-use-hooks-classes-or-a-mix-of-both) 당신이 다른 이유가 있거나 얼리어답터가 되는 것을 신경 쓰지 않는다면 당신의 이미 존재하는 컴포넌트들을 다시 작성하세요. Hooks 은 여전히 새롭고 (마치 React의 2014년 같습니다.), 그리고 일부 “모범 사례들”이 아직 튜토리얼에 포함되지 않았습니다.

So where does that leave us?
그럼 그게 우리를 떠나는건가요? 리액트 함수와 클래스 사이ㅔ 근본적이 차이점이 있나요? 물론이죠, 그것들은 — 정신 모델(?)에 있습니다. **이 게시물에서, 저는 그들 사이의 가장 큰 차이점을 볼 것입니다.** 그것은 함수형 컴포넌트가 [2015년에 소개된 이래](https://reactjs.org/blog/2015/09/10/react-v0.14-rc1.html#stateless-function-components)로 존재 했지만 종종 간과되었습니다:
